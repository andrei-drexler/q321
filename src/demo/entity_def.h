...
#define DEMO_ENTITY_TYPES_WEAPONS(x)\
	...
	x(weapon_gauntlet,				"Gauntlet",				Model::gauntlet_barrel,		  0,   0,   0)\
	x(weapon_machinegun,			"Machinegun",			Model::machinegun_barrel,	  0,   0,   0)\
	x(weapon_shotgun,				"Shotgun",				Model::shotgun,				  0,   0,   0)\
	x(weapon_rocketlauncher,		"Rocket launcher",		Model::rocketl,				  0,   0,   0)\
	x(weapon_railgun,				"Railgun",				Model::railgun,				  0,   0,   0)\
	x(weapon_plasmagun,				"Plasma Gun",			Model::plasma,				  0,   0,   0)\

#define DEMO_ENTITY_TYPES_AMMO(x)\
	/*Name							Description				Model						  R,   G,   B*/\
	x(ammo_bullets,					"Bullets",				Model::rocketam,			128, 112,   2)\
	x(ammo_rockets,					"Rockets",				Model::rocketam,			128,   2,   2)\
	x(ammo_shells,					"Shells",				Model::rocketam,			128,  64,   2)\
	x(ammo_slugs,					"Slugs",				Model::rocketam,			  0,  80,  32)\
	x(ammo_cells,					"Cells",				Model::rocketam,			112,   0,  64)\

#define DEMO_ENTITY_TYPES_HEALTH(x)\
	/*Name							Description				Model						  R,   G,   B*/\
	x(item_health,					"25 Health",			Model::large_cross,			160, 128,  48)\
	x(item_health_large,			"50 Health",			Model::large_cross,			144,  88,  56)\
	x(item_health_mega,				"Mega Health",			Model::mega_cross,			  8, 119, 234)\

#define DEMO_ENTITY_TYPES_ARMOR(x)\
	/*Name							Description				Model						  R,   G,   B*/\
	x(item_armor_combat,			"Armor",				Model::armor_red,			255, 255,   0)\
	x(item_armor_shard,				"Armor Shard",			Model::shard,				 80,  88,  86)\
	x(item_armor_body,				"Heavy Armor",			Model::armor_red,			255,   0,  16)\

#define DEMO_ENTITY_TYPES_POWERUPS(x)\
	/*Name							Description				Model						  R,   G,   B*/\
	x(item_quad,					"Quad Damage",			Model::quad,				  8, 119, 234)\
